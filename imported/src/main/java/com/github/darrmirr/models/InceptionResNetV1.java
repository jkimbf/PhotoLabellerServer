...
public class InceptionResNetV1 implements Dl4jModel {
    ...
    public void init() throws IOException {
        ...
        if (!weightsResource.exists()) {
            ...
            String weightsDirPath = Paths.get(WEIGHTS_PATH).getParent().toString();
            ClassPathResource weightsDirResource = new ClassPathResource(weightsDirPath);

            TreeSet<File> splittedFilesSet = Optional
            ...
            LinkedList<InputStream> splittedInputStreams = new LinkedList<InputStream>();
            for (File file : splittedFilesSet) {
                splittedInputStreams.add(new FileInputStream(file));
            }

            try(SequenceInputStream inputStream = new SequenceInputStream(enumeration(splittedInputStreams))) {
                Path weightsPath = Paths.get(weightsDirResource.getFile().getAbsolutePath(), weightsResource.getFilename());
