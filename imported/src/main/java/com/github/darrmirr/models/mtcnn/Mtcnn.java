...
public class Mtcnn {
    ...
    public List<BoundBox> detectFaces(INDArray img) {
        ...
        INDArray scaledImage = Nd4jUtils.scale(img, optimizedScaleSize);
        List<BoundBox> boundBoxes = Optional
        ...
        if(originalHeight > optimizedScaleSize || originalWidth > optimizedScaleSize) {
            double scale = originalHeight > originalWidth ? (double) originalHeight / optimizedScaleSize : (double) originalWidth / optimizedScaleSize;
    ...
    private List<BoundBox> reScale(List<BoundBox> boundBoxes, double scale, int originalHeight, int originalWidth){
        return boundBoxes
                .stream()
                .map(box -> {
                    BoundBox originalBbox = mtcnnUtils.scale(box, scale);
